...
class rproxyMessage extends daFsm {
    ...
    request(message) {
        try {
            ...
            .then((res) => {
                logger.trace(`Receive Response: ${res.id}`);
                return res;
            })  
    ...
    requestPrepare(message,callback) {
        ...
        if (callback != undefined && 
            ...
            logger.trace(`Origin Sending Request: ${request.id}`);
        }
    ...
    responseProcess(msg) {
        if (msg.id != undefined && 
            ...
            logger.trace(`Origin Response Processing: ${msg.id}`);
        } else {
    ...
    responseProceed(msg) {
        ...
        if (command != undefined && msg.id != undefined &&
            ...
            logger.trace(`Target Response Proceeding: ${msg.id}`);    
            this.msgSend({
                type:'utf8',
               ...
               ,body: this.onMessage[command](msg)
            })});
        } else {
